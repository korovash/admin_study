...
#### Лёгкие
- Скачайте ISO образ и создайте виртуальную машину в VirtualBox.
...
- Узнайте IP адрес ВМ и попробуйте выполнить ping до этого адреса. Если не получается, попытайтесь разобраться с типами сетевых интерфейсов в ВМ и подобрать рабочий вариант.
- Подключитесь по ssh с помощью учетной записи admin с паролем средствами командной строки и консольного клиента вашей ОС.
- Подключитесь по ssh с помощью учетной записи admin с паролем средствами стороннего клиента ssh (putty/kitty/mobaxterm).
- Подключитесь по scp с помощью учетной записи admin с паролем средствами стороннего клиента scp (WinSCP) и скопируйте произвольный файл на хост в домашнюю директорию
...
- Добавить доменное имя для ВМ, которое будет разрешаться в IP адрес машины на вашем устройстве. Выполнить пинг по этому имени.
- Скопируйте произвольный файл на хост в домашнюю директорию средствами scp консольного клиента в терминале с помощью учетной записи admin с паролем.
...
- Настройте сервис `sshd` так, чтобы root не мог подключаться по SSH. Если это сделал установщик ОС, просто выполните проверку конфига на предмет этой настройки.
